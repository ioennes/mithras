...
from tensorflow.keras.layers import Dense, MaxPool1D, Input
...
def model_summary_to_json(model):
    ...
    model_config = {"model_name": f"dnn{count}", "input_shape": [dim for dim in model.input_shape if dim is not None],
                    "layers": []}
    count += 1
